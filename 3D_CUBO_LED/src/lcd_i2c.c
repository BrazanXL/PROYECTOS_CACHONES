...
void lcd_send_cmd(char cmd)
{
	...
	data_h = (cmd&0xf0);
	data_l = ((cmd<<4)&0xf0);
	data_t[0] = 0x0C|data_h;  //en = 1, rs = 0
	data_t[1] = 0x08|data_h;  //en = 0, rs = 0
	data_t[2] = 0x0C|data_l;  //en = 1, rs  =0
	data_t[3] = 0x08|data_l;  //en = 0, rs = 0
...
void lcd_write_data(char data)
{
	...
	data_h = (data&0xf0);
	data_l = ((data<<4)&0xf0);
	data_t[0] = 0x0D|data_h;  //en = 1, rs = 0
	data_t[1] = 0x09|data_h;  //en = 0, rs = 0
	data_t[2] = 0x0D|data_l;  //en = 1, rs  =0
	data_t[3] = 0x09|data_l;  //en = 0, rs = 0
	err = i2c_master_write_to_device(I2C_LCD_PORT, I2C_LCD_SLAVE_ADDRESS, data_t, 4, 1000);
	if (err != 0) ESP_LOGI(TAG, "Error no. %d in command", err);
}
